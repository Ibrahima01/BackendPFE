...
public class RendezVousServiceImpl implements RendezVousService {
    ...
    public int tempPrelevement(Patient p){
        if (p.getAge()<=2)
            return parametreService.getParametre().getDUREE_PRELEVEMENT_AGE_0_2();
        else {
            if (p.getAge()<=12)
                return parametreService.getParametre().getDUREE_PRELEVEMENT_AGE_2_12();
            else {
                    if (p.isObese())
                        return parametreService.getParametre().getDUREE_PRELEVEMENT_OBESE();
                    else
                        return parametreService.getParametre().getDUREE_PRELEVEMENT_AGE_PLUS12();
                }
            }
    }
    /*
    ...
    public RendezVous ajoutRdv(Patient p){
        int numBox=(rendez_vousRepository.findAll().get(rendez_vousRepository.findAll().size()-1).getNumero_box()+1)%parametreService.getParametre().getNOMBRE_BOX();
        if (numBox == 0) {
            numBox = parametreService.getParametre().getNOMBRE_BOX();
        }
    ...
    @Override
    public RendezVous updateNumBox(Long id, int nombreBox){
        /*
        int numBox=(rendez_vousRepository.findAll().get(rendez_vousRepository.findAll().size()-1).getNumero_box()+1)%nombreBox;
        if (numBox == 0) {
            numBox = parametreService.getParametre().getNOMBRE_BOX();
        }
        */
        RendezVous rendezVous=findRDVByPatient(id);
        if (rendezVous.getNumero_box()>nombreBox)
            rendezVous.setNumero_box(nombreBox);
        //rendezVous.setDate_heure_RDV(DernierHeurRdvBox(numBox).plusMinutes(TempPrelDernierPatiBox(numBox)));
        rendez_vousRepository.save(rendezVous);
        return rendezVous;
    }
}
